...
using namespace std;
using namespace cv;
void writeShow(JNIEnv *env, jobject listener, jmethodID method, int resType, cv::Mat paramMat = Mat(), vector<int> paramInt = {}, vector<double> paramDouble = {}, vector<string> paramString = {}) {
	typedef void(*Write)(JNIEnv *env, jobject listener, jmethodID method, int resType, cv::Mat paramMat, vector<int> paramInt, vector<double> paramDouble, vector<string> paramString);
	HINSTANCE Dll2 = LoadLibraryA("server/src/main/resources/Write.dll");
	if (Dll2 == NULL)
	{
		cout << "load dll2 failed" << endl;
		//imgHandle(paraStr, env, listener, method);
		std::cout << GetLastError() << endl;
	}
	Write myWrite = (Write)GetProcAddress(Dll2, "Write");
	if (myWrite == NULL)
	{
		cout << "load func2 failed" << endl;
	}
	myWrite(env, listener, method, resType, paramMat, paramInt, paramDouble, paramString);
}
...
public:
	...
	static std::unordered_map<int, TreeNode*> m_nodeMap;
	static JNIEnv *env;
	...
	virtual void Run() {
		...
		if (cando) {
			...
			if (m_name == "imgFilter") {
				HMODULE module = LoadLibrary("server/src/main/resources/GaussianBlur.dll");     // 根据DLL文件名，加载DLL，返回一个模块句柄
				if (module == NULL)
				{
			...
			else if (m_name == "dataInput") {
				HMODULE module = LoadLibrary("server/src/main/resources/Read.dll");     // 根据DLL文件名，加载DLL，返回一个模块句柄
				if (module == NULL)
				{
					cout << "load dll failed" << endl;
					return;
			...
			//resMap[this->m_id] = mat;//将执行结果存入哈希表，其他结果根据函数类型自己写在上面的判断里面
			else if (m_name == "outputFile") {
				string paramMName = para_string[0];
				string paramResType = para_string[1];
				//通过 paraMName找到该节点，然后拿到id
				/*find m_id*/
				int m_id;
				std::unordered_map<int, TreeNode*>::iterator it;
				for (it = m_nodeMap.begin(); it != m_nodeMap.end(); it++)
				{
					TreeNode* node = it->second;
					if (node->m_name == paramMName) {
						m_id = node->m_id;
						break;
					}
				}
				//找不到id
				if (it == m_nodeMap.end()) {
					cout << "not find node id" << endl;
					throw std::runtime_error("not find node id");
				}
				...
				int mapping_id = mapping->find(paramResType);
				switch (mapping_id) {
					//Mat
				case 0:
				{std::unordered_map<int, cv::Mat>::iterator it = resMap.find(m_id);
				if (it != resMap.end()) {
					//在resMap中找得到
					cv::Mat mat = Mat();
					mat = it->second;
					//调用Write()函数
					writeShow(env, listener, method, mapping_id, mat);
				}
				else {
					//在resMap中找不到
					cout << "no Mat result" << endl;
				}
				}
					break;
					//Int
...
std::unordered_map<int, TreeNode*> TreeNode::m_nodeMap;
JNIEnv *TreeNode::env;
...
TreeNode* CreateNode(const json& nodeJson, std::unordered_map<int, TreeNode*>& nodeMap) {
	...
	std::string dllname = properties["modelName"].get<std::string>();
	...
	for (const auto& para : inPara) {
		node->addpara(para["from"].get<std::string>());
	}
	nodeMap[id] = node;
	return node;
}
